...
def get_encoded_image(file_prefix, images_path):
    ...
    """
    filtered_files = [file for file in images_path if file.startswith(file_prefix)]
...
def add_image_to_prompt(encoded_images):
    ...
    input_token_count = 4 + 2 # Assuming 4 tokens for the <imsystem> message and 2 tokens for the <imassistant> message
...
def save_response_as_md(response, output_folder, batch_num):
    """
    Saves the response as a Markdown (.md) file.
    """
    os.makedirs(output_folder, exist_ok=True)
    md_file_path = os.path.join(output_folder, f"batch_{batch_num + 1}.md")
